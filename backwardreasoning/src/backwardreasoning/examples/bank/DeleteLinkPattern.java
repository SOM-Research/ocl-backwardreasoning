...
//		this.variableNameSource = variableNameTarget;
...
//		System.err.println("Delete link pattern");
//		for(PropertyCallExp<EClassifier, EStructuralFeature> item : result)
//		if (((EClass)item.getReferredProperty().getEType()).getName().equals(eClassTarget.getName())) {
//		
...
//			variableTarget.setName(variableNameSource);
//			variableTarget.setType(eClassSource);
...
//			item.setName("+");
//			operationCallExp.setReferredOperation(plus);
//			operationCallExp.getArgument().add(variableSource);
//			operationCallExp.setSource(item);
//			IfExp ifExp = oclFactory.createIfExp();
//			ifExp.setCondition(item);
//			
//			
//			OperationCallExp operationCallExp = oclFactory.createOperationCallExp();
